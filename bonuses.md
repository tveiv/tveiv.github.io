...
.filter-btn.active[data-filter="green"] { background: #2e7d32; color: #fff; border-color: #2e7d32; box-shadow: 0 0 10px rgba(46, 125, 50, 0.4); }
...
.filter-btn.active[data-filter="set"] { background: #9c27b0; color: #fff; border-color: #9c27b0; box-shadow: 0 0 10px rgba(156, 39, 176, 0.4); }
...
/* Рамки и подсветки под редкости */
...
/* ===== 3. ХАЙЛАЙТЫ ЦИФР И ПРОЦЕНТОВ (ЗОЛОТОЙ СТИЛЬ) ===== */
.card-desc strong, .card-desc b, .card-desc u {
    color: #ffd54f !important;
    font-weight: bold;
    text-shadow: 0 0 5px rgba(255, 213, 79, 0.2);
}
...
Добро пожаловать в наиболее полную интерактивную энциклопедию всех бонусов, меток сущностей (комплектных сетов) и проклятий в игровом режиме **VoidGame на Cristalix**. 

Воспользуйтесь поиском по названию/описанию или удобной панелью фильтрации по редкости, чтобы быстро найти точные параметры любого эффекта!
    ...
    <div class="bonuses-grid" id="bonuses-grid">
        ...
        <div class="bonus-card rarity-{{ bonus.rarity }}" data-category="{{ bonus.rarity }}" data-name="{{ bonus.name | downcase }}" data-desc="{{ bonus.description | downcase }}">
            <div>
                ...
                <!-- Используем jekyll filter markdownify для корректного рендеринга жирного шрифта из JSON -->
                <p class="card-desc">{{ bonus.description | markdownify }}</p>
            </div>
